...
"""
1.为自己写一个函数注意事项以供后续参考。你可以写在一个索引卡片上随时阅读，直到你记住所有的要点为止。注意事项如下：

	1.函数定义是以 def 开始的吗？
		是的。函数代码块以 def 关键词开头，后接函数标识符名称和圆括号()。
	2.函数名称是以字符和下划线 _ 组成的吗？
		错，函数名称是由字母，数字，下划线组成，不能以数字开头
	3.函数名称是不是紧跟着括号 ( )？
		是的，函数代码块以 def 关键词开头，后接函数标识符名称和圆括号()。
...
2.当你运行（或者说“使用 use”或者“调用 call”）一个函数时，记得检查下面的要点：
	...
	4.函数是否以 () 结尾？
	5.按照这两份检查表里的内容检查你的练习，直到你不需要检查表为止。
